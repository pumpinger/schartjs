    ...
    var s={
        ...
        isHome:false,
        fontColor:'#b0b0b0',
        _init:function(svg,data,nodataStr){
            ...
            if ( typeof  data !="object" ) {
                console.log('s:非法数据');
                return s._errImage(svg,nodataStr);
            }else{
                for(var i in data){
                    return true;
                }
        ...
        drawLine:function(dom,data,legend,fun,option,nodataStr) {
            var svg = Snap(dom);
            ...
            function drawOneTip(index,tipdata,isK2){
                ...
                var text=svg.paper.text(poiontx,isK2?2*tb-tt-(tb-tt-14)/ 2:tb-(tb-tt-14)/2, [tipdata.name+':　',"￥",s._formatMoney(tipdata.price)+"　",tipdata.num,"笔"]).attr({
            ...
            function _drawBgLeft(){
                ...
                for (var i = 1; i <= cline; i++) {
                    ...
                    svg.text(cl, h + 13, s._formatMoney(cperv * i)).attr({
                        fill:isHome?'#7abfe9':s.fontColor,
                        fontSize: 10
                    });
                }
            }
        ...
        drawBar:function(dom,data,legend,fun,option,nodataStr){
            ...
            function drawOneTip(index,tipdata,k2data){
                if(tipdata){
                    ...
                    if(!dataFix){
                        var text=svg.paper.text(poiontx,tb-(tb-tt-14)/2,[legend[0]+':　',"￥", s._formatMoney(tipdata.price)+"　",tipdata.num,"笔"]).attr({
                            fill: "#fff",
                            fontSize: 14
                        });
                    ...
                    if(k2data){
                        ...
                        var text2=svg.paper.text(poiontx,2*tb-tt-(tb-tt-14)/2,[legend[1]+':　',"￥",s._formatMoney(k2data.price)+"　",k2data.num,"笔"]).attr({
                            fill: "#fff",
                            fontSize: 14
                        });
        ...
        drawColumn:function(dom,data,legend,fun,option,nodataStr){
            var svg = Snap(dom);
...
//                        console.log(parenttext.node.getComputedTextLength());
        ...
        drawPie:function(dom,data,legend,fun,option,nodataStr){
            ...
            function drawOneLegend(color,i,v,pie,movex,movey){
                if( isfull){
                    ...
                    var text=svg.text(vw/10+cloumn*(vw*0.8)/fullColumn+10,cb+38+row*30, s._formatName(v.name,fullColumn==3?5:8)).attr({fontSize:14});
                }else{
            ...
            function drawTip(color,v){
                ...
                if(typeof  data[0].time == 'undefined'){
                    ...
                    var text=svg.text(vw/2,tb-((tb-tt)-10)/2, [legend[0]+':　', v.time,'个客户']).attr({fill:'#fff','class':"svgpietip",fontSize:14});
        ...
        drawMixAvg:function(dom,data,legend,fun,option,nodataStr){
            {
                var svg = Snap(dom);
                var columnSvg,columnl,cn;  //column 的坐标系 column 左边距离 column的数量
                ...
                var vw = s._getParam(option,'vw',$(dom).width()), vh = s._getParam(option,'vh',$(dom).height());  //viwebox 宽高
                var tt = s._getParam(option,'tt',0),tb = s._getParam(option,'tb',30);  //提示框离头部距离 提示框底部
                var ct =s._getParam(option,'ct',2*(tb-tt)+tt+10), cb=vh-25,cl = 10, cr = vw;   //图表区域  top  bottom  left right
        ...
        drawPcColumn:function(dom,data,legend,fun,nodataStr){
            s.fontColor="#333333";
            s.isPc=true;

            var option= s._pcScreen(dom);

            s.drawColumn(dom,data,legend,fun,option,nodataStr);
        },

        drawPcLine:function(dom,data,legend,fun,nodataStr){

            s.fontColor="#333333";
            s.isPc=true;
            var option= s._pcScreen(dom);

            s.drawLine(dom,data,legend,fun,option,nodataStr);
        },


        drawPcBar:function(dom,data,legend,fun,nodataStr){
            s.fontColor="#333333";
            s.isPc=true;
            var option= s._pcScreen(dom);

            s.drawBar(dom,data,legend,fun,option,nodataStr);
        },

        drawPcPie:function(dom,data,legend,fun,nodataStr){
            s.fontColor="#333333";
            s.isPc=true;

            $(dom).removeAttr('viewBox');

            s.drawPie(dom,data,legend,fun,nodataStr);
        },


        drawFullColumn:function(dom,data,legend,fun,nodataStr){
        ...
        drawFullMixAvg:function(dom,data,legend,fun,nodataStr){
            var v=s._fullScreen(dom,340);
            var vw= v.width;
            var vh= v.height;
            ...
            s.drawMixAvg(dom,data,legend,fun,option,nodataStr);
        },
        drawFullLine:function(dom,data,legend,fun,nodataStr){
        ...
        drawFullBar:function(dom,data,legend,fun,nodataStr){
        ...
        drawFullPie:function(dom,data,legend,fun,nodataStr){
            var v=s._fullScreen(dom);
            var vw= v.width;
            var vh= v.height;
            ...
            s.drawPie(dom,data,legend,fun,option,nodataStr);
        },
        drawHomeLine:function(dom,data,legend,fun,nodataStr){
            s.isHome=true;
            var option={
                isHome:true,
                tt:20,
        ...
        _errImage:function (svg,nodataStr){
            var svgw,svgh;
            if(svg.attr('viewBox')){
